...
namespace POL {
	...
	polContainer *polarizationBase::polCon = NULL;
	pcdContainer *polarizationBase::pcdCon = NULL;

	polarizationBase::polarizationBase()
	{
	}
	...
	void polarizationBase::padImage(const cv::Mat input, cv::Mat &output, int pad_x, int pad_y, double val) {
	...
	void  polarizationBase::imgSplit(const cv::Mat &input, vector<cv::Mat>&output, int method) {
	...
	void polarizationBase::imgSave(const cv::Mat &img, const string &filepath, const string& format) {
